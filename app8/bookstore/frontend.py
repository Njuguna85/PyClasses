...
def search_command():
    listbox1.delete(0, END)
    for row in db.search_book(title_entry.get(), author_entry.get(), year_entry.get(), isbn_entry.get()):
        listbox1.insert(END, row)
...
def delete_command():
    db.delete_book(selected_tuple[0])
